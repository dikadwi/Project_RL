...
admin_pelatihan_bp = Blueprint(
    'admin_pelatihan', __name__, url_prefix='/admin/pelatihan')
...
def get_next_pelatihan_id():
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cur.execute(
        "SELECT MAX(CAST(SUBSTRING(id, 2) AS UNSIGNED)) AS max_id FROM pelatihan")
    row = cur.fetchone()
    cur.close()
...
@admin_pelatihan_bp.route("/", methods=["GET"])
@login_required
def index():
    ...
    cursor.execute(
        "SELECT * FROM pelatihan ORDER BY CAST(SUBSTRING(id,2) AS UNSIGNED)")
    pelatihan_data = cursor.fetchall()
    cursor.close()

    next_id = get_next_pelatihan_id()
    ...
    return render_template(
        "admin/admin_pelatihan_list.html",
        pelatihan=pelatihan_data,
        next_id=next_id,
        vark_opts=vark_opts,
        mlsq_opts=mlsq_opts,
        ams_opts=ams_opts,
...
@admin_pelatihan_bp.route("/save", methods=["POST"])
@login_required
def save_pelatihan():
    if current_user.role != "admin":
        return "Unauthorized", 403

    _id = request.form.get("id") or get_next_pelatihan_id()
    judul = request.form.get("judul")
    deskripsi = request.form.get("deskripsi")
    kategori = request.form.get("kategori")
    skor_poin = request.form.get("skor_poin") or 0
    kode_action = request.form.get("kode_action") or 106
    ...
    sql = """
        INSERT INTO pelatihan (id, judul, deskripsi, kategori, skor_poin, target_state, kode_action)
    ...
    return redirect(url_for('admin_pelatihan.index'))


@admin_pelatihan_bp.route("/delete/<string:id>", methods=["POST"])
@login_required
def delete_pelatihan(id):
    if current_user.role != "admin":
        return "Unauthorized", 403

    cursor = mysql.connection.cursor()
    cursor.execute("DELETE FROM pelatihan WHERE id = %s", (id,))
    mysql.connection.commit()
    cursor.close()
...
@admin_pelatihan_bp.route("/upload", methods=["POST"])
@login_required
def upload_csv_pelatihan():
    if current_user.role != "admin":
        return "Unauthorized", 403
    ...
    try:
        ...
        flash(f"✅ Berhasil impor {len(rows)} pelatihan dari CSV.")
